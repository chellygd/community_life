...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    var $ = layui.$
        ...
        , thisTable = function () {
            ...
            var o = e.templet ? function () {
                if ("function" == typeof e.templet) {
                    ...
                    laytpl($(e.templet).html() || String(a)).render(l);
                }
            }() : a;
            return n ? $("<div>" + o + "</div>").text() : o
        }
        ...
        , ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed',
        ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_TOTAL = '.layui-table-total', ELEM_PAGE = '.layui-table-page',
        ...
        TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
            ...
            , '{{# if(d.data.skin){ }}'
            , ' lay-skin="{{d.data.skin}}"'
            , '{{# } }} '
            , '{{# if(d.data.size){ }}'
            , ' lay-size="{{d.data.size}}"'
            , '{{# } }}'
            , '{{# if(d.data.even){ }}'
            , ' lay-even'
            , '{{# } }}'
    ...
    Class.prototype.render = function () {
        ...
        //展开行配置
        if (options.treeExpand === false || Math.floor(options.treeExpand) <= 0) {
            options.treeExpand = 1;
        }
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    if (res.data === null || res.data === undefined || res.data === "null" || res.data === "undefined"
            ...
            "object" == typeof options.totalRow && (res[response.totalRowName] = $.extend({}, options.totalRow))
    ...
    Class.prototype.filterArray = function (data, field_Id, field_upId) {
        ...
        var fa2 = function (l, level, upids) {
            for (var i = 0; i < l.length; i++) {
                ...
                if (options.treeExpand !== true && Math.floor(options.treeExpand) <= Math.floor(level)) {
                    n.isOpen = false;
                }
                n.upIds = upids;
                tableList.push(n);
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            ...
            , total_row_name = res[options.response.totalRowName]
            , trs = []
            , trs_fixed = []
            , trs_fixed_r = []
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    //行数据
                    var oRowData = data[i1];
                    //展开和收起图标
                    var iconExpend = "&#xe625;";//默认展开
                    if (options.treeExpand !== true && Math.floor(options.treeExpand) <= Math.floor(oRowData.level)) {
                        iconExpend = "&#xe623;";//收起图标
                    }
                    //收起的行要隐藏
                    var rowNoExpend = "";
                    if (options.treeExpand !== true && Math.floor(options.treeExpand) < Math.floor(oRowData.level)) {
                        rowNoExpend = ' style="display:none!important;" ';
                    }
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        if (treeShowName == item3.field) {//当前是否用于显示的值
                            ...
                            var temTreeHtml = '<i class="layui-icon layui-tree-head extend-icon" style="cursor: pointer;">' + iconExpend + '</i> ';

                            var nbspHtml = "<i>"//一次位移
                            for (var i = 1; i < oRowData.level; i++) {
                                nbspHtml = nbspHtml + "&nbsp;&nbsp;&nbsp;&nbsp;";
                            }
                            nbspHtml = nbspHtml + "</i>";

                            if (oRowData.children && oRowData.children.length > 0) {//非叶子节点
                    ...
                    trs.push('<tr ' + rowNoExpend + ' data-index="' + i1 + '" upids="' + item1["upIds"] + '">' + tds.join('') + '</tr>');
        ...
        that.renderTotal(data, total_row_name);
    ...
    //合计行
    Class.prototype.renderTotal = function (e, t) {
        var that = this, option = that.config, l = {};
        if (option.totalRow) {
            layui.each(e, function (e, t) {
                0 !== t.length && that.eachCols(function (e, i) {
                    var a = i.field || e, n = t[a];
                    i.totalRow && (l[a] = (l[a] || 0) + (parseFloat(n) || 0))
                })
            }), that.dataTotal = {};
            var n = [];
            that.eachCols(function (e, o) {
                var r = o.field || e, d = function () {
                    var e = o.totalRowText || "", a = {}, i;
                    if (o.totalRowType != undefined && o.totalRowType === "int") {
                        i = parseInt(l[r]).toString();
                    } else {
                        i = parseFloat(l[r]).toFixed(2);
                    }
                    return a[r] = i, i = u(o, i, a), t ? t[o.field] || e : o.totalRow ? i || e : e
                }(), c = ['<td data-field="' + r + '" data-key="' + that.index + "-" + o.field + '" ' + function () {
                    var e = [];
                    ...
                    var e = that.index + "-" + o.field;
                    return "normal" === o.type ? e : e + " laytable-cell-" + o.type
                }() + '">' + d, "</div></td>"].join("");
                o.field && (that.dataTotal[r] = d), n.push(c)
            }), that.layTotal.find("tbody").html("<tr>" + n.join("") + "</tr>")
        }
    }
    ...
    Class.prototype.scrollPatch = function () {
        ...
        if (scollWidth && scollHeight) {
            if (!that.elem.find('.layui-table-patch')[0]) {
                ...
                var patchElem1 = $('<td class="layui-table-patch"><div class="layui-table-cell"></div></td>'); //补丁元素
                patchElem1.find('div').css({width: scollWidth});
                that.layTotal.eq(0).find('tr').append(patchElem1);
            }
        } else {
            that.layHeader.eq(0).find('.layui-table-patch').remove();
            that.layTotal.eq(0).find('.layui-table-patch').remove();
        }
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'i.layui-tree-head', function () {
            ...
            var dbClickI2 = $("[upids*=_" + o[options.treeId] + "_]").find('i.extend-icon');
            if (o.isOpen) {//打开状态
                dbClickI.html('&#xe625;');
                dbClickI2.html('&#xe625;');
            } else {
